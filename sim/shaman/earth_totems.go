...
func (shaman *Shaman) newStrengthOfEarthTotemSpellConfig(rank int) core.SpellConfig {
	...
	strengthOfEarthTotemAuras[0] = core.StrengthOfEarthTotemAura(&shaman.Unit, shaman.Level, multiplier, core.CharacterBuildPhaseNone)
	if hasFeralSpirit {
		strengthOfEarthTotemAuras[1] = core.StrengthOfEarthTotemAura(&shaman.SpiritWolves.Unit, shaman.Level, multiplier, core.CharacterBuildPhaseNone)
	}

	spell := shaman.newTotemSpellConfig(ClassSpellMask_ShamanStrengthOfEarthTotem, spellId, manaCost)
...
func (shaman *Shaman) newStoneskinTotemSpellConfig(rank int) core.SpellConfig {
	...
	spell := shaman.newTotemSpellConfig(ClassSpellMask_ShamanStoneskinTotem, spellId, manaCost)
	spell.RequiredLevel = level
	spell.Rank = rank
	spell.ApplyEffects = func(sim *core.Simulation, _ *core.Unit, spell *core.Spell) {
		shaman.TotemExpirations[EarthTotem] = sim.CurrentTime + duration
...
func (shaman *Shaman) registerTremorTotemSpell() {
	...
	spell := shaman.newTotemSpellConfig(ClassSpellMask_ShamanTremorTotem, spellId, manaCost)
	spell.RequiredLevel = level
	spell.ApplyEffects = func(sim *core.Simulation, _ *core.Unit, spell *core.Spell) {
		shaman.TotemExpirations[EarthTotem] = sim.CurrentTime + duration
		shaman.ActiveTotems[EarthTotem] = spell
