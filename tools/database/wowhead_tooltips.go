...
var hit2Regex = regexp.MustCompile(`Improves your chance to hit with all spells and attacks by ([0-9]+)%\.`)
...
func (item WowheadItemResponse) GetStats() Stats {
	...
	return Stats{
		...
		proto.Stat_StatSpellHit:          float64(item.GetIntValue(hitRegex) + item.GetIntValue(hit2Regex) + item.GetIntValue(spellHitRegex)),
		proto.Stat_StatMeleeHit:          float64(item.GetIntValue(hitRegex) + item.GetIntValue(hit2Regex) + item.GetIntValue(physicalHitRegex)),
		proto.Stat_StatSpellCrit:         float64(item.GetIntValue(critRegex) + item.GetIntValue(spellCritRegex)),
		proto.Stat_StatMeleeCrit:         float64(item.GetIntValue(critRegex) + item.GetIntValue(meleeCritRegex)),
